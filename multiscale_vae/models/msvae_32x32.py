...
class img_conditional_decoder(nn.Module):
    ...
    def forward(self, input):
        ...
        d1 = self.upc1(torch.cat([h5, z], 1))
        d2 = self.upc2(torch.cat([d1, h4], 1))
        d3 = self.upc3(torch.cat([d2, h3], 1))
        d4 = self.upc4(torch.cat([d3, h2], 1))
        output = self.upc5(torch.cat([d4, h1], 1))
        if self.out:
